...
import asyncio
from data_transform.core_utils import extract_pdf_data, save_text_to_csv, generate_csv_from_docs
import logging
...
async def main(username: str, password: str, accounts_list: list[str], output_directory: str):
    ...
    async with client:
        ...
        for account_no in accounts_list:
            try:
                ...
                extracted_data = await extract_pdf_data(f'{doc_id}.pdf')

                # Step 10: Save extracted text to CSV TODO: here we need to insert the data to the file if it is already exist
                csv_data = save_text_to_csv(output_directory, extracted_data)
