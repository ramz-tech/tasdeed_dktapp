...
def load_accounts(path):
    ...
    try:
        ...
        if "SUBTYPE" in df.columns:
            ...
            raise KeyError("The 'SUBTYPE' column is missing in the input file.")

        return df.to_dict(orient="records")
    except FileNotFoundError:
        raise FileNotFoundError(f"The file at path '{path}' was not found.")
    except Exception as e:
        raise RuntimeError(f"An error occurred while loading accounts: {e}")
...
def generate_bill(account):
    ...
    try:
        ...
        raise KeyError(f"Missing required account field: {e}")
    except Exception as e:
        raise RuntimeError(f"An error occurred while generating the bill: {e}")
...
def save_to_file(data, path):
    ...
    try:
        ...
        raise PermissionError(f"Permission denied: Unable to write to file '{path}'.")
    except Exception as e:
